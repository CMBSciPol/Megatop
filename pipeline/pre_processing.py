...
def MakeSims(args):
    ...
    if meta_sim.map_sim_pars['cmb_sim_no_pysm']:
        ...
        Cl_BB_lens = meta_sim.map_sim_pars['A_lens']*Cl_lens[2]
        Cl_TT = Cl_lens[0]
        Cl_EE = Cl_lens[1]
        Cl_TE = Cl_lens[3]
        ...
        cmb_sky = hp.synfast([Cl_TT, Cl_EE, Cl_BB, Cl_TE, Cl_EE*0.0, Cl_EE*0.0], 
                             nside=meta_sim.map_sim_pars['nside_sim'], new=True)
    else:
    ...
    for f in range(len(meta.general_pars['frequencies'])):
        ...
        CMB_fg_alms = hp.map2alm(CMB_fg_freq_maps[f], lmax=3*meta_sim.map_sim_pars['nside_sim'])

        Bl_gauss_fwhm = hp.gauss_beam( np.radians(meta.pre_proc_pars['fwhm'][f]/60), lmax=3*meta_sim.map_sim_pars['nside_sim'])
                
        for alm_ in CMB_fg_alms:
            hp.almxfl(alm_, Bl_gauss_fwhm, inplace=True)             

        CMB_fg_freq_maps_beamed.append(hp.alm2map(CMB_fg_alms, meta_sim.map_sim_pars['nside_sim']) )        
...
def plotTTEEBB_diff(meta, meta_sims, Cl_data, Cl_model, save_name, legend_labels=[r'label data $C_\ell$ $\nu=$', r'label model $C_\ell$ $\nu=$'], axis_labels=['y_axis_row0', 'y_axis_row1']):
...
def check_sims(args, cmb_sky, noise_maps, freq_maps, fg_freq_maps, fsky_binary):
        meta_sims = BBmeta(args.sims)
        meta = BBmeta(args.globals)
        ...
        plotTTEEBB_diff(meta, meta_sims, cl_noise_f, model_noise, 'Noise_lvl_check.png', 
                        legend_labels=[r'Noise $C_\ell$ from map $\nu=$', r'Input white noise lvl $\nu=$'], 
                        axis_labels=[r'$D_\ell \, [\mu K \, rad]^2$', r'$\Delta_{\ell} / Input_\ell$'])

        Cl_cmb_model = get_Cl_CMB_model_from_meta(args)

        plotTTEEBB_diff(meta, meta_sims, np.array([cl_cmb_sky_maps]), Cl_cmb_model[...,:cl_cmb_sky_maps.shape[-1]], 'CMB_check.png', 
                        legend_labels=[r'$C_{\ell}^{\rm CMB}$ from map', r'Input $C_{\ell}^{\rm CMB}$'], 
                        axis_labels=[r'$D_\ell \, [\mu K \, rad]^2$', r'$\Delta_{\ell} / Input_\ell$'])        


def check_preproc(args, preproc_freq_maps, fg_freq_maps, fsky_binary):
    # meta_sims = BBmeta(args.sims)
    meta = BBmeta(args.globals)
    ...
    cl_fg_freq_maps = np.array(cl_fg_freq_maps)
    cl_preproc_freq_maps = np.array(cl_preproc_freq_maps)
    ...
    beam_correction_T_freq = []
    beam_correction_P_freq = []
    for f in range(len(meta.general_pars['frequencies'])):
        #beam corrections
        Bl_gauss_fwhm = hp.gauss_beam( np.radians(meta.pre_proc_pars['fwhm'][f]/60), lmax=lmax_convolution, pol=True)
        ...
        beam_correction_T_freq.append(sm_corr_T)
        beam_correction_P_freq.append(sm_corr_P)
    ...
    CMB_fg_beamed_spectra_TT = (Cl_cmb_model + cl_fg_freq_maps) * beam_correction_NODECONV_T**2
    CMB_fg_beamed_spectra_EE = (Cl_cmb_model + cl_fg_freq_maps) * beam_correction_NODECONV_P**2
    CMB_fg_beamed_spectra_BB = (Cl_cmb_model + cl_fg_freq_maps) * beam_correction_NODECONV_P**2

    # Noise after pre-processing model: 
    model_noise = get_Nl_white_noise(args, fsky_binary)
    model_noise_beamed = model_noise * np.array([beam_correction_NODECONV_T**2, beam_correction_NODECONV_P**2, beam_correction_NODECONV_P**2])

    model_beamed_total = model_noise_beamed + np.array([CMB_fg_beamed_spectra_TT, CMB_fg_beamed_spectra_EE, CMB_fg_beamed_spectra_BB])
...
if __name__ == "__main__":
    ...
    if args.sims:
        ...
        if not mpi:
            ...
            for sim_num in range(meta_sims.general_pars['nsims']):
                ...
                print('simulating maps sim number: ', sim_num, '/', meta_sims.general_pars['nsims'])
                ...
                np.save(os.path.join(meta_sims.output_dirs['root'], meta_sims.output_dirs['comb_directory'], 'comb_freq_maps_SIM'+str(sim_num).zfill(5)+'.npy' ), freq_maps )
                np.save(os.path.join(meta_sims.output_dirs['root'], meta_sims.output_dirs['cmb_directory'], 'cmb_maps_SIM'+str(sim_num).zfill(5)+'.npy' ), cmb_sky )
                np.save(os.path.join(meta_sims.output_dirs['root'], meta_sims.output_dirs['cmb_beamed_directory'], 'cmb_beamed_freq_maps_SIM'+str(sim_num).zfill(5)+'.npy' ), CMB_fg_freq_maps_beamed )
                np.save(os.path.join(meta_sims.output_dirs['root'], meta_sims.output_dirs['fg_directory'], 'fg_freq_maps_SIM'+str(sim_num).zfill(5)+'.npy' ), fg_freq_maps )
                np.save(os.path.join(meta_sims.output_dirs['root'], meta_sims.output_dirs['noise_directory'], 'noise_freq_maps_SIM'+str(sim_num).zfill(5)+'.npy' ), noise_maps )
        if mpi:
            print('TODO!!')
    ...
    if args.sims and args.plots:
        ...
        check_sims(args, cmb_sky, noise_maps, freq_maps, fg_freq_maps, fsky_binary)
    ...
    if not mpi:
        ...
        if args.sims and args.plots:
            ...
            # This only checks the last simulations
            # TODO: in MPI do all cases or just one? 
            check_preproc(args, freq_maps_common_beamed, cmb_sky, noise_maps, fg_freq_maps, fsky_binary)
