...
def GetNoiseCov(args):
    ...
    if args.sims:
        if meta.noise_cov_pars["noise_cov_method"] == "load_sims":
            ...
            ) = wrapper_load_sim_noise_cov(args, meta, mpi, rank, size, root, comm)

            if args.plots and rank == root:
                binary_mask_from_nhits_preproc = wrapper_plotting(
                    args,
                    meta,
                    rank,
                    noise_cov_mean,
            ...
            if args.plots and rank == root:
                wrapper_plotting_matrices_and_maps(
                    args,
                    meta,
                    rank,
                    noise_cov_mean,
        ...
        noise_cov_preprocessed_mean, freq_noise_maps_pre_processed, nside_in_list = (
            wrapper_cov_noise_from_noise_maps(args, meta, mpi)
        )

        if args.plots:
            wrapper_plotting_noise_cov_from_noise_maps(
                args,
                meta,
                noise_cov_preprocessed_mean,
...
def wrapper_load_sim_noise_cov(args, meta, mpi, rank, size, root, comm):
    ...
    noise_cov = np.zeros(
        [len(meta.frequencies), 3, hp.nside2npix(meta_sims.map_sim_pars["nside_sim"])]
    )
    ...
    if not mpi:
        ...
        if meta_sims.general_pars["nsims"] != size:
            exit(
                "ERROR: nsims must be equal to size in MPI mode. nsims = "
                + str(meta_sims.general_pars["nsims"])
                + "  size = "
                + str(size)
            )
    ...
    for sim_num in range(nsims_iter):
        ...
        if args.verbose:
            print(
                "NOISE COV ESTIMATION LOADING EXTERNAL NOISE-ONLY MAPS, SIM#",
                sim_num + 1,
                "/",
                nsims,
            )
        freq_noise_maps = np.load(
            os.path.join(
                meta_sims.noise_directory, "noise_freq_maps_SIM" + str(sim_num).zfill(5) + ".npy"
            )
        )
    ...
    if rank == root:
        ...
        np.save(os.path.join(meta.covmat_directory, "pixel_noise_cov.npy"), noise_cov_mean)
        np.save(
    ...
    if mpi:
        if args.verbose:
            print(
                "MPI Gathering freq_noise_maps_pre_processed_array and freq_noise_maps_array to root, rank=",
                rank,
            )

        MemoryUsage(args, f"rank = {rank} ")

        freq_noise_maps_pre_processed_array = MPIGATHER(
            freq_noise_maps_pre_processed_array[0], comm, rank, size, root
        )
        freq_noise_maps_array = MPIGATHER(freq_noise_maps_array[0], comm, rank, size, root)

        if args.verbose:
            print(
                "MPI Gathering freq_noise_maps_pre_processed_array and freq_noise_maps_array to root DONE, rank=",
                rank,
            )
...
def wrapper_sim_in_noisecov(args, meta, mpi, rank, root, comm=None):
    ...
    if args.verbose:
        print("noise_cov_method = sim_in_noisecov")
    MemoryUsage(args, f"rank = {rank} ")
    ...
    if not mpi:
        ...
        for sim_id in range(nsims):
            ...
            for map_name in maps_list:
                ...
                freq_noise_maps.append(
                    MakeNoiseMapsNhitsMSS2(meta, map_name, verbose=args.verbose).tolist()
                )
            MemoryUsage(args, f"rank = {rank} ")
        ...
        if args.verbose:
            print("Time taken for noise_cov_preprocessed = ", time.time() - start)
...
def wrapper_cov_noise_from_noise_maps(args, meta, mpi):
    ...
    if hasattr(meta, "ben_sims") and meta.ben_sims:
        ...
        nside_in_list = [
            hp.npix2nside(freq_noise_maps_array.shape[-1])
        ] * freq_noise_maps_array.shape[0]
    else:
        for m in maps_list:
            if args.verbose:
                print("Importing map: ", m)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sims", default=None, help="Generate a set of sims if True.")
