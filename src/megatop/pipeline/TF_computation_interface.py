...
def create_and_run_soopercool_yaml(manager: DataManager, config: Config, id_sim: int | None = None):
    base_config = {
    ...
    'transfer_settings': {
        'transfer_directory': str(manager.path_to_transfer_functions_parents / Path('transfer_functions_output')),
        'couplings_dir': str(manager.path_to_transfer_functions_parents / Path('couplings_output')),
    ...
    for map_set in config.map_sets:
        variable_config.append({
            ...
            'transfer_settings':{
                'unfiltered_map_dir': {
                    f'{map_set.name}': str(manager.path_to_TF_sims_maps)
                },
                'unfiltered_map_template':{
                    f'{map_set.name}': "{id_sim:04d}/simforTF_{pure_type}_"+f"{map_set.name}_unfiltered.fits"
                },
                'filtered_map_dir':{
                    f'{map_set.name}': str(manager.path_to_TF_sims_maps)
                },
                'filtered_map_template':{
                    f'{map_set.name}': "{id_sim:04d}/simforTF_{pure_type}_"+f"{map_set.name}_filtered.fits"
                },
            }
        })

    full_soopercool_config_path_list = []
    for var, map_set in zip(variable_config, config.map_sets):
        # soopercool_config = base_config.copy()
        # soopercool_config.update(var)
        ...
        with open(full_soopercool_config_path, 'w') as f:
    ...
    for path_soop_config in full_soopercool_config_path_list:
        ...
        pseudo_cell_script = soopercool_path/Path('pipeline/transfer/compute_pseudo_cells_tf_estimation.py')
        subprocess.run(['python', str(pseudo_cell_script), '--globals', path_soop_config, '--verbose'])
    ...
    for i, path_soop_config in enumerate(full_soopercool_config_path_list):
        transfer_functions_script = soopercool_path/Path('pipeline/transfer/compute_transfer_function.py')
        subprocess.run(['python', str(transfer_functions_script), '--globals', path_soop_config])
        logger.info(f"Transfer Functions computed for {config.map_sets[i].name} and saved to {TF_paths_checks[i]}")
