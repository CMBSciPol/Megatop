...
def Cl_CMB_model(
    ...
    if not dust_marg and not sync_marg:
        ...
        Birefringence = (np.pi/180)*Birefringence # Conversion degres -> radians
    ...
    #Birefringence mixing E and B modes
    CL_EE_obs = (np.cos(2*Birefringence)**2)*Cl_EE + (np.sin(2*Birefringence)**2)*Cl_BB_CMB
    CL_BB_obs = (np.cos(2*Birefringence)**2)*Cl_BB_CMB + (np.sin(2*Birefringence)**2)*Cl_EE
    CL_EB_obs = 0.5*(np.sin(4*Birefringence))*(Cl_EE-Cl_BB_CMB)
    CL_BE_obs = 0.5*(np.sin(4*Birefringence))*(Cl_EE-Cl_BB_CMB)
...
def prior_bounds(theta, dust_marg, sync_marg, prior_bounds_dict):
    ...
    if not dust_marg and not sync_marg: # A
        r, A_lens, Birefringence = theta
        if (lower_bound_r <= r <= upper_bound_r) and (
            lower_bound_A_lens <= A_lens <= upper_bound_A_lens
        ) and (
            lower_bound_Birefringence <= Birefringence <= upper_bound_Birefringence
        ):
            return 0.0
    if dust_marg and not sync_marg:
        r, A_lens, Birefringence, A_dust = theta
...
def logL_cosmo(
    ...
    C_est = np.array([[Cl_CMBxCMB_EE_est, Cl_CMBxCMB_EB_est],
...
def run_mcmc_and_save(manager: DataManager, config: Config, id_sim: int | None = None):
    ...
    try:
        Cl_CMBxCMB_EE_est = np.load(manager.get_path_to_spectra_cross_components(id_sim))[
            "CMBxCMB"
        ][0]
        Cl_CMBxCMB_BB_est = np.load(manager.get_path_to_spectra_cross_components(id_sim))[
            "CMBxCMB"
        ][3]
        Cl_CMBxCMB_EB_est = np.load(manager.get_path_to_spectra_cross_components(id_sim))[
            "CMBxCMB"
        ][1]
        Cl_CMBxCMB_BE_est = np.load(manager.get_path_to_spectra_cross_components(id_sim))[
            "CMBxCMB"
        ][2]
        #Cl_DustxDust_EE_est = np.load(manager.get_path_to_spectra_cross_components(id_sim))[
        #    "DustxDust"
        #][0]
        Cl_DustxDust_BB_est = np.load(manager.get_path_to_spectra_cross_components(id_sim))[
            "DustxDust"
        ][3]
        Cl_DustxDust_EB_est = np.load(manager.get_path_to_spectra_cross_components(id_sim))[
            "DustxDust"
        ][1]
        ...
        if np.all(np.array(all_noise_options) == NoiseOption.NOISELESS):
            ...
            Nl_CMBxCMB_EE_est = np.load(manager.get_path_to_noise_spectra_cross_components(id_sim))[
            "Noise_CMBxNoise_CMB"
            ][0]
            Nl_CMBxCMB_BB_est = np.load(manager.get_path_to_noise_spectra_cross_components(id_sim))[
                "Noise_CMBxNoise_CMB"
            ][3]
            Nl_CMBxCMB_EB_est = np.load(manager.get_path_to_noise_spectra_cross_components(id_sim))[
                "Noise_CMBxNoise_CMB"
            ][1]
            Nl_CMBxCMB_BE_est = np.load(manager.get_path_to_noise_spectra_cross_components(id_sim))[
                "Noise_CMBxNoise_CMB"
            ][2]
    ...
    if config.cl2r_pars.load_model_spectra:
        ...
        Cl_EE, Cl_BB_prim_generic, Cl_BB_lensing_generic = compute_generic_Cl(
            0, 2 * config.nside + config.map2cl_pars.delta_ell
        )
    # 2. init mcmc parameters:
    if not dust_marg and not sync_marg:
        param_names = ["r", "A_{lens}", "Birefringence"] # A
        theta_init_guess = [0.005, 0.5, 0.35]
        theta_offsets = [0.005, 0.1, 0.1]
    if dust_marg and not sync_marg:
        param_names = ["r", "A_{lens}", "Birefringence", "A_{dust}"]
        theta_init_guess = [0.005, 0.5, 0.35, 0.001]
        theta_offsets = [0.005, 0.1, 0.1, 0.001]
    if not dust_marg and sync_marg:
