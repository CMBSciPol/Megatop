...
def mask_handler(args):
        ...
        if not os.path.exists(meta.masks["input_nhits_path"]):
            ...
            if meta.filtering_type == "toast":
                ...
                raise FileNotFoundError(
                    "Cannot find nhits file {}".format(meta.masks["input_nhits_path"])
                )
    ...
    if args.plots:
        ...
        hp.graticule()
        nhits_save_path = os.path.join(plot_dir, meta.masks["nhits_map"])
    ...
    binary_mask = get_binary_mask_from_nhits(
        nhits, meta.nside, zero_threshold=meta.masks["mask_handler_binary_zero_threshold"]
    )
    ...
    if not meta.use_input_nhits:
        ...
        if "galactic" in meta.masks["include_in_mask"]:
            ...
            for id_key, gal_key in enumerate(gal_keys):
                ...
                meta.timer.stop(f"gal{id_key}", f"Galactic mask {gal_key} projection", args.verbose)
                ...
                if args.plots:
                    ...
                    hp.graticule()
                    plt.savefig(
                        os.path.join(plot_dir, fname.split("/")[-1].replace(".fits", ".png"))
                    )
        ...
        if "point_source" in meta.masks["include_in_mask"]:
            ...
            if os.path.isfile(ps_fname):
                ...
                meta.timer.stop("ps_mask", "Load point source mask from disk", args.verbose)
            ...
            if args.plots:
                ...
                hp.graticule()
                plt.savefig(
                    os.path.join(plot_dir, ps_fname.split("/")[-1].replace(".fits", ".png"))
                )
        ...
        meta.timer.stop("final_mask", "Assembling final analysis mask", args.verbose)
