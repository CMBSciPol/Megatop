...
def make_sims(meta, verbose=True, plots=False, noise_only=False):
    ...
    if meta.noise_sim_pars["noise_option"] == "white_noise":
        ...
        for i_f, f in enumerate(instrument.frequency):
            nlev_map[i_f] = np.array(
                [instrument.depth_i[i_f], instrument.depth_p[i_f], instrument.depth_p[i_f]]
            )[:, np.newaxis] * np.ones((3, hp.nside2npix(nside)))
        nlev_map /= hp.nside2resol(nside, arcmin=True)
        noise_maps = np.random.normal(
            np.zeros((len(meta.frequencies), 3, hp.nside2npix(nside))),
            nlev_map,
            (len(meta.frequencies), 3, hp.nside2npix(nside)),
        )
    ...
    if meta.noise_sim_pars["noise_option"] != "no_noise" and meta.noise_sim_pars["include_nhits"]:
        ...
        try:
            noise_maps[..., np.where(binary_mask_sim == 1)[0]] /= np.sqrt(
                nhits_map_rescaled[np.where(binary_mask_sim == 1)[0]]
            )
            ...
            print(
                'Please check the mask_handling parameters, changing "mask_handler_binary_zero_threshold" can help.'
            )
            print("Exiting...")
            exit()
    ...
    if noise_only:
        if verbose:
            print("Simulating noise only maps")
    ...
    if meta.map_sim_pars["cmb_sim_no_pysm"]:
        if plots:
            ...
            plt.legend()
            plt.savefig(
                os.path.join(meta.plots_directory, "fiducial_CMB_spectra.png"), bbox_inches="tight"
            )
            plt.close()
        ...
        if meta.map_sim_pars["fixed_cmb"]:
            ...
            np.random.seed(0)
        cmb_map = hp.synfast(
            Cl_cmb_model[0],  # [Cl_TT, Cl_EE, Cl_BB, Cl_TE, Cl_EE*0.0, Cl_EE*0.0],
            nside=nside,
            new=True,
            pixwin=False,
        )
    ...
    fg_freq_maps = np.array(
        [rot_gal2ec.rotate_map_alms(fg_freq_maps_freq) for fg_freq_maps_freq in fg_freq_maps]
    )
    ...
    for i_f, f in enumerate(meta.frequencies):
        ...
        cmb_fg_alms_T, cmb_fg_alms_Q, cmb_fg_alms_U = hp.map2alm(
            cmb_fg_freq_maps[i_f], lmax=lmax_convolution, pol=True
        )
        Bl_gauss_fwhm = hp.gauss_beam(
            np.radians(beams_FWHM[i_f] / 60), lmax=lmax_convolution, pol=True
        )
        ...
        # alm-->mapf
        cmb_fg_alms_out_T, cmb_fg_alms_out_Q, cmb_fg_alms_out_U = hp.alm2map(
    ...
    return (
        cmb_fg_noise_freq_maps,
        cmb_fg_freq_maps_beamed,
        noise_maps,
    )  # TODO check if cmb_fg_noise_freq_maps is needed or not ...
...
def get_noise_beams(meta, fsky_binary, verbose=True):
    ...
    if meta.noise_sim_pars["experiment"] == "SO":
        ...
        if meta.noise_cov_pars["noise_cov_method"] == "sim_in_noisecov":
            print(
                "WARNING: When using SO as experiment, for the noise covariance step one should be carefull when using noise_cov_method = sim_in_noise_cov"
            )
            print(
                "Please make sure the noise lvl in noise_cov_pars.noise_lvl_uKarcmin is the same as the one used in the simulation."
            )
            print("A solution is to use MSS2 experiment which will use consistent noise lvl")

    if meta.noise_sim_pars["experiment"] == "MSS2":
        print(
            "WARNING: When using MSS2 as experiment, ouptut map_noise_lvl from get_noise_beams() should be ignored."
        )
...
def get_NL_from_white_noise(meta, map_white_noise_levels):
    ...
    for i_f in range(len(meta.frequencies)):
        N_ell_white_f_arcmin.append(np.ones(lmax_convolution) * map_white_noise_levels[i_f] ** 2)
...
def plotTTEEBB_diff(
    ...
    for f in range(Cl_data.shape[0]):
        ...
        ax[0][2].plot(
            ...
            + str(meta.general_pars["frequencies"][f]) * (Cl_data.shape[0] != 1),  #
            color="C" + str(f),
            ls="-",
            alpha=0.4,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--plots", action="store_true", help="Plot various outputs maps if True.")
    parser.add_argument(
    ...
    cmb_fg_noise_freq_maps, cmb_fg_freq_maps_beamed, noise_maps = make_sims(
