...
def so_V3_LA_beams():
    ...
    beam_LAC_280 = 0.9
    return np.array(
        [beam_LAC_27, beam_LAC_39, beam_LAC_93, beam_LAC_145, beam_LAC_225, beam_LAC_280]
    )
...
def so_V3_LA_noise(sensitivity_mode, f_sky, ell_max, delta_ell=1, beam_corrected=False):
    ...
    NTubes_UHF = 2.0
    # sensitivity
    S_LA_27 = np.array([61, 48, 35]) * np.sqrt(
        1.0 / NTubes_LF
    )  ## converting these to per tube sensitivities
    S_LA_39 = np.array([30, 24, 18]) * np.sqrt(1.0 / NTubes_LF)
    S_LA_93 = np.array([6.5, 5.4, 3.9]) * np.sqrt(4.0 / NTubes_MF)
    S_LA_145 = np.array([8.1, 6.7, 4.2]) * np.sqrt(4.0 / NTubes_MF)
    S_LA_225 = np.array([17, 15, 10]) * np.sqrt(2.0 / NTubes_UHF)
    S_LA_280 = np.array([42, 36, 25]) * np.sqrt(2.0 / NTubes_UHF)
    ...
    LA_beams = so_V3_LA_beams() / np.sqrt(8.0 * np.log(2)) / 60.0 * np.pi / 180.0
    ...
    N_ell_T_LA = np.array(
        [N_ell_T_27, N_ell_T_39, N_ell_T_93, N_ell_T_145, N_ell_T_225, N_ell_T_280]
    )
...
def so_V3_SA_beams():
    ...
    beam_SAC_280 = 9.0
    return np.array(
        [beam_SAC_27, beam_SAC_39, beam_SAC_93, beam_SAC_145, beam_SAC_225, beam_SAC_280]
    )
...
def so_V3_SA_noise(
    ...
    # configuraiton
    if SAC_yrs_LF > 0:
        NTubes_LF = SAC_yrs_LF / 5.0 + 1e-6  ## reguarlized in case zero years is called
        NTubes_MF = 2 - SAC_yrs_LF / 5.0
    else:
        NTubes_LF = -SAC_yrs_LF / 5.0 + 1e-6  ## reguarlized in case zero years is called
        NTubes_MF = 2
    NTubes_UHF = 1.0
    # sensitivity
    S_SA_27 = np.array([32, 21, 15]) * np.sqrt(1.0 / NTubes_LF)
    S_SA_39 = np.array([17, 13, 10]) * np.sqrt(1.0 / NTubes_LF)
    S_SA_93 = np.array([4.6, 3.4, 2.4]) * np.sqrt(2.0 / (NTubes_MF))
    S_SA_145 = np.array([5.5, 4.3, 2.7]) * np.sqrt(2.0 / (NTubes_MF))
    S_SA_225 = np.array([11, 8.6, 5.7]) * np.sqrt(1.0 / NTubes_UHF)
    S_SA_280 = np.array([26, 22, 14]) * np.sqrt(1.0 / NTubes_UHF)
    # 1/f pol:  see http://simonsobservatory.wikidot.com/review-of-hwp-large-aperture-2017-10-04
    f_knee_pol_SA_27 = np.array([30.0, 15.0, 1.0, 90.0])
    f_knee_pol_SA_39 = np.array([30.0, 15.0, 1.0, 90.0])  ## from QUIET
    f_knee_pol_SA_93 = np.array([50.0, 25.0, 1.0, 150.0])
