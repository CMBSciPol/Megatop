...
def megabuster_precomputations(manager: DataManager, config: Config):
    ...
    if np.all(np.array(obsmat_operator_fname) == Path()):
        ...
        logger.debug(f"Build observation matrix to be used in CG and saved in {obsmat_operator_fname}")
        
        print("Starting saving obsmat precomputation...", flush=True)
        mb.precomputations.save_obsmat_precomputation(list_path_obsmat_input=list_path_obsmat_files, list_path_obsmat_output=obsmat_operator_fname)
