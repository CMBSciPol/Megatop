...
def bandlimit_maps(
    ...
    """
    Bandlimit maps to lmax by doing a map2alm→alm2map cycle.
    Corrects only for the pixel window function change (nside_in → nside_out).
    No beam correction is applied.
    """
    nside_input_maps = [hp.npix2nside(m.shape[-1]) for m in freq_maps]
    if any(n < nside for n in nside_input_maps):
        raise ValueError("Some input maps have smaller nside than target. Check your yaml.")
    ...
    logger.info(f"Bandlimiting maps to lmax={lmax} and NSIDE -> {nside}, no beam correction.")

    wpix_out = hp.pixwin(nside, pol=True, lmax=lmax)  # Pixel window function of output maps

    for i_map, freq_map in enumerate(freq_maps):
        # Input window function. WARNING: Must be done in the loop, as input maps don't necessarily have the same nside
        wpix_in = hp.pixwin(
            nside_input_maps[i_map],
            pol=True,
            lmax=lmax,
            datapath=HEALPY_DATA_PATH,
        )
        ...
        # Only pixel window correction, no beam correction
        sm_corr_T = wpix_out[0] / wpix_in[0]
        sm_corr_P = wpix_out[1] / wpix_in[1]
